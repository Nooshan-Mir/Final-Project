...
setwd(dir = "/Users/nooshanmir/Documents/Purdue Courses/Spring 2024 (2nd Semester)/ASNC 516/Microbiome Final Project/Microbiome_Data_From_Annabel/FinalFiles2")
...
head(final_results)
ggplot(gut_ASVs, aes(x = ASV8, y = ASV10)) +
  geom_point()
...
write.csv(strong_results_taxa, "output/FinalProject-strong-results-taxa.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="CECUM"), "output/FinalProject-strong-results-taxa-CECUM.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="COLON"), "output/FinalProject-strong-results-taxa-COLON.csv", row.names = F)

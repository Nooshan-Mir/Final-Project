...
#Phyloseq is for finding phylogenetic diversity which shows relative abundance=>taxabar plot
#and Deseq showen the most abundant ASVs in the samples=>ancom
...
#colorbrewer2.oprg where you can bring the codes for colors
my_colors <- c(
...
#You can change it but these are the stndards
my_level <- c("Phylum", "Family", "Genus")
#appears on X.axis (what you are intersted)
my_column <- "Diet"  #this is the metadata column that we will use in the taxa barplot

rm(taxa.summary)
#less than this level will be excluded
...
for(ml in my_level){
  ...
  physeq_meta_filtered$Diet = factor(physeq_meta_filtered$Diet, c("AC", "NAC"))
